...
def compute_per_token_logprobs(
    ...
    *,
    ...
    logprobs = -F.cross_entropy(shift_logits, target, reduction='none')
    logprobs = logprobs.view(input_ids.size(0), -1)
    return logprobs
...
def build_completion_mask(
    ...
    mask = attention_mask[:, 1:].clone()  # [B, L-1], shift to align with per-token logprobs
    mask[:, :prompt_input_len-1] = 0  # zero out prompt tokens
    return mask.float()
