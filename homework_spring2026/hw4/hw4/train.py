...
def parse_args() -> TrainConfig:
    ...
    ap.add_argument(
        "--skip_initial_eval",
        action=argparse.BooleanOptionalAction,
        default=TrainConfig.skip_initial_eval,
    )
    ap.add_argument(
        "--skip_final_eval",
        action=argparse.BooleanOptionalAction,
        default=TrainConfig.skip_final_eval,
    )
...
def compute_group_advantages(rewards: torch.Tensor, group_size: int, eps: float = 1e-6) -> torch.Tensor:
    ...
    if rewards.numel() % group_size != 0:
        raise ValueError(f"Expected rewards.numel() to be divisible by group_size, got {rewards.numel()} and {group_size}")
    
    
    reward = rewards.view(-1,group_size)
